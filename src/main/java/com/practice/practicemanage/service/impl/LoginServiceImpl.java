...
//    private TypeConversionUtil typeConversionUtil;
    ...
    public ResponseMessage<Object> login(UserLoginDto users) {
        ...
        if (StringUtil.isEmpty(userName) || StringUtil.isEmpty(password)) {
            logUtil.error(LoginServiceImpl.class, "用户名或密码不能为空");
        ...
        if (user == null || user.getStatus() == 0) {
            logUtil.error(LoginServiceImpl.class, "用户名或密码错误");
            return ResponseMessage.error("用户名或密码错误");
        } else if (user.getStatus() == 5) {
            logUtil.info(LoginServiceImpl.class, "请通知管理员启用权限");
            return ResponseMessage.error("请通知管理员启用权限");
        }
...
//    private UnitUserRepository unitUserRepository;
    ...
    public ResponseMessage<Object> logouts(String token, String refreshToken) {
        try {
            ...
            logUtil.error(LoginServiceImpl.class, "无令牌退出");
            return ResponseMessage.success("令牌不存在，登出成功");
        } catch (Exception e) {
            logUtil.error(LoginServiceImpl.class, "登出失败", e);
    ...
    public ResponseMessage<Object> register(UserRegisterDto user) {
        try {
            if (isExist(user.getUserName(), user.getPhone())) {
                System.out.println("用户名或手机号已存在");
                return ResponseMessage.error("用户名或手机号已存在");
            }
...
//            }
            ...
            logUtil.error(LoginServiceImpl.class, "注册失败", e);
            return ResponseMessage.error("注册失败");
        }
    }

    public boolean isExist(String userName, String phone) {
        System.out.println("查询结果返回："+userRepository.existsByUserNameOrPhone(userName, phone));
        return userRepository.existsByUserNameOrPhone(userName, phone);
    }
...
//        保存用户账号密码（登录用）
    ...
    public ResponseMessage<Object> infoByToken(String token, List<Menu> menuList) {
        try {
            logUtil.info(LoginServiceImpl.class, "获取用户信息");
            User user = (User) redisUtil.get("REFRESHTOKEN_"+token+jwtUtil.extractUsername(token));
            if (user == null) {
                logUtil.error(LoginServiceImpl.class, "用户不存在");
...
//            }
            ...
            if (Objects.equals(role.getIdentity(), "STUDENT")) {
                ...
                logUtil.error(LoginServiceImpl.class, "该用户无角色");
                return ResponseMessage.error("该用户无角色");
            }

        } catch (Exception e) {
            logUtil.error(LoginServiceImpl.class, "获取用户信息失败", e);
    ...
    public ResponseMessage<Object> getScoolORUnitList(Integer type) {
        try {
            ...
            logUtil.error(LoginServiceImpl.class, "获取信息失败", e);
